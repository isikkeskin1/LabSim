...
`labsim.sensitivity_variance` attributes scalar-response variance across independent uncertain inputs. `variance_sensitivity` constructs two independent Monte Carlo matrices and one hybrid matrix per parameter. It reports a Saltelli-style first-order index and a Jansen total-effect index for every named parameter. First-order effects quantify variance explained by a parameter alone; total effects also include interactions involving that parameter.

Sensitivity analysis deliberately accepts an explicit scalar observable over parameter mappings instead of being coupled to one model type. A caller can therefore analyze an analytical response or wrap a complete LabSim simulation and extract final state, peak response, event time, energy drift, or another scientific quantity. The estimator currently assumes independent marginals; applying these indices directly to correlated designs would change their interpretation and is intentionally unsupported by this API.

Independent and correlated sampling are separate, explicit concepts. Latin hypercube sampling and the current variance-attribution estimator target independent marginals only; preserving dependence requires dedicated statistical designs rather than silently reordering or hybridizing correlated draws.
