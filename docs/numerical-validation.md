...
The harmonic oscillator is the first reference problem because

`x(t) = cos(ωt)` and `v(t) = -ω sin(ωt)`

for the unit-amplitude, zero-velocity initial condition at `ω = 1`.
